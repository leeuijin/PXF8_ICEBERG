...
====================================================================================================================================
 방식                  | 동작 원리                                                          | 장점                           | 단점                                
====================================================================================================================================
 Copy-on-Write (CoW)   | 변경 대상이 포함된 기존 Parquet 파일을 메모리로 읽어온 뒤,         | 조회(Read) 시 바로 새 파일만   | 쓸 때(Write)마다 파일 전체를        
                       | 수정/삭제를 반영하여 완전히 새로운 Parquet 파일을 통째로           | 읽으면 되므로 쿼리 속도가       | 다시 쓰므로 쓰기 비용이 매우 큽니다.
                       | 다시 씁니다.                                             | 가장 빠릅니다.               |                                     
------------------------------------------------------------------------------------------------------------------------------------
 읽기 시 병합 (MoR)       | 기존 원본 Parquet 파일은 그대로 둡니다. 대신 어떤 행(Row)이       | 쓰기 속도가 매우 빠르고        | 조회할 때 원본 파일과 삭제 파일을   
                       | 지워졌거나 수정되었는지만 기록하는 작은 삭제 파일(Delete File)을    | 효율적입니다. 스트리밍 환경에    | 실시간으로 병합(Merge)해야 하므로   
                       | 새로 생성합니다.                                          | 적합합니다.                 | 쿼리 속도가 다소 느려집니다.        
====================================================================================================================================
